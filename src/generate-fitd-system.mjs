...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'fitd', 'Fitd', and 'FITD'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.fitd/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.fitd/g),
   ...
   * Rename files.
   *
   * Rename files that had fitd in their name, such as
   * css/fitd.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*fitd*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('fitd', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-fitd-system.mjs`);
